	...
	if ($_SESSION['signuser'] == 0 || $_SESSION['permission'] == 0){
		...
		if($_SESSION['admin_auth'] == 1 || $_SESSION['mod_auth'] == 1){
			...
			if ($role == 1 || $role ==2 || $role == 3){	
					...
					if ($update_result > 0){	
						...
						if ($admin_check_value['username'] == $pres_user ){					
							if ($admin_check_value['role'] != "admin"){
								...
								header('Location:http://www.cs.odu.edu/~cpallapo/CS518/proj4/members.php');
							}					
						}
					...
					if ($update_result > 0){
						header('Location:http://www.cs.odu.edu/~cpallapo/CS518/proj4/pagination.php');
					...
					if($mailsent){
						...
						if ($update_result > 0){
							header('Location:http://www.cs.odu.edu/~cpallapo/CS518/proj4/memstats.php?name='.$user);
						}
					}			
			}
			else if ($role == 6){
				...
				if ($update_result > 0){
					header('Location:http://www.cs.odu.edu/~cpallapo/CS518/proj4/imageLimit.php');
				}
			}
			else{
				header('Location:http://www.cs.odu.edu/~cpallapo/CS518/proj4/index.php');
